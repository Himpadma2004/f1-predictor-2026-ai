...
def _process_single_driver(args):
    ...
    driver_no, laps, driver_code = args

    print(f"Getting telemetry for driver: {driver_code}")
    
    # Use the already-loaded laps data passed from main process
    laps_driver = laps.pick_drivers(driver_no)
...
def get_race_telemetry(session, year, round_number, session_type="R"):
    ...
    # Load laps ONCE in main process - much faster than reloading in each worker
    print("Loading session laps (this may take a moment)...")
    laps_start = time.time()
    laps = session.laps
    print(f"✓ Laps loaded in {time.time()-laps_start:.1f}s ({len(laps)} laps)")
    
    # Prepare arguments for parallel processing - pass laps directly
    print(f"Processing {len(drivers)} drivers in parallel...")
    driver_args = [
        (driver_no, laps, driver_codes[driver_no]) for driver_no in drivers
    ]
    ...
    print(f"✓ Multiprocessing completed in {time.time()-processing_start:.1f}s")
    ...
    print("The replay should begin in a new window shortly")
    print(f"✓ Total telemetry processing completed in {time.time()-total_start:.1f}s")
    return {
        "frames": frames,
        "driver_colors": get_driver_colors(session),
        "track_statuses": formatted_track_statuses,
