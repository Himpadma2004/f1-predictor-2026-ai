...
# Track spawned threads for cleanup
_spawned_threads = []
...
def launch_telemetry_viewer():
  ...
  viewer_thread = threading.Thread(target=start_viewer, daemon=False)
  viewer_thread.start()
  _spawned_threads.append(viewer_thread)
...
def launch_insights_menu():
  ...
  menu_thread = threading.Thread(target=start_menu, daemon=False)
  menu_thread.start()
  _spawned_threads.append(menu_thread)


def wait_for_spawned_threads(timeout=5):
  """Wait for all spawned threads to complete."""
  for thread in _spawned_threads:
    try:
