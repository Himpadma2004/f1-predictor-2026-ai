...
class RaceSelectionWindow(QMainWindow):
    ...
    def closeEvent(self, event):
        """Properly clean up threads and resources before closing."""
        try:
        ...
        try:
            if hasattr(self, '_session_worker') and self._session_worker and self._session_worker.isRunning():
                self._session_worker.quit()
                self._session_worker.wait(timeout=2000)
        except Exception:
            pass
        
        try:
            if hasattr(self, '_ready_timer') and self._ready_timer:
